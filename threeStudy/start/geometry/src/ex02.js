...
// ----- 주제: Geometry 기본
//geometry segment를 쪼개서 멋진 효과를 적용시킬수도있음
  ...
  // positionArray[i] = positionArray[i] + ( Math.random() - 0.5 ) * 0.2;
    ...
    const time = clock.getElapsedTime() * 3;
      ...
      // positionArray[i+2] += Math.sin(time + randomArray[i+2] * 100) * 0.001;
		...
		positionArray[0] += Math.sin(time + randomArray[0] * 100) * 0.001;
		positionArray[1] += Math.sin(time + randomArray[0] * 100) * 0.001;
		positionArray[2] += Math.sin(time + randomArray[0] * 100) * 0.001;
